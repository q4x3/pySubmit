    ...
    if (tmp.is<std::string>()) {
        ...
        if (str2 == std::string("None")) {
            ...
            if (tel(str2).is<std::string>()) {
                ...
                if (tel(str2).as<std::string>() == std::string("None")) {
                    std::cout << "None";
                    return;
                }
            }
            ...
            if (s > 1) {
                ...
                for (int i = 0;i < atm0.size();++ i) {
                    if (atm0[i].is<std::string>()) {
                        ...
                        if (tmp[0] != '"' && tmp != std::string("None")) atm0[i] = tel(tmp);
                        if (tmp == "None") atm0[i] = std::string("None");
                    }
                }
    ...
    antlrcpp::Any visitIf_stmt(Python3Parser::If_stmtContext *ctx) override {
        ...
        for (int i = 0;i < siz;++ i) {
            ...
            if (transbool(tmp)) {
                ...
                if (tmp1.is<vector<antlrcpp::Any>>()) return tmp1;
                cnt = 1; 
                break;
            }
        }
    ...
    antlrcpp::Any visitSuite(Python3Parser::SuiteContext *ctx) override {
        ...
        if (s > 0)
            for (int i = 0;i < s;++ i) {
                ...
                if (tmp.is<std::string>() && tmp.as<std::string>() == std::string("break")) return tmp;
                if (tmp.is<std::string>() && tmp.as<std::string>() == std::string("continue")) return tmp;
                if (tmp.is<vector<antlrcpp::Any>>()) return tmp;
            }
        if (ctx->simple_stmt()) {
